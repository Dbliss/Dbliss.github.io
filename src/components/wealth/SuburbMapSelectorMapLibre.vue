...
const currentChunkMeta = computed(() => {
  const active = availableStates.value.find(entry => entry.code === activeState.value)
...
watch(mapReady, async (ready) => {
  if (ready) await loadActiveChunk()
})

watch(currentChunkMeta, async () => {
  await loadActiveChunk()
})
...
async function loadActiveChunk() {
  ...
  if (!currentChunkMeta.value) {
    statusMessage.value = availableStates.value.length
      ? `No generated boundary chunk is available for ${activeState.value}.`
      : 'No suburb geometry manifest is available yet.'
...
function findChunkForState(stateCode) {
  ...
  const match = entries.find(chunk => chunk?.state === stateCode || chunk?.id === stateCode.toLowerCase())
